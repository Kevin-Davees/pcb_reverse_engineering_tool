...
class AppState:
    ...
    def push_undo(self, stage: int, snap):
        ...
        # Cap undo depth to prevent memory explosion on large images
...
class Stage1_Load(BaseStage):
    ...
    def _open_image(self):
        ...
        if not path:
            return
        try:
            Image.MAX_IMAGE_PIXELS = 40_000_000  # guard against decompression bombs
            S.raw = Image.open(path).convert("RGBA")
            w, h = S.raw.size
            if w * h > 40_000_000:
                messagebox.showwarning(
                    "Image Too Large",
                    f"Image is {w}×{h} = {w*h:,} pixels. "
                    "This may cause memory issues. Consider downscaling first."
                )
    ...
    def _load_project(self):
        ...
        try:
            ...
            if not isinstance(pins,   list): raise ValueError("'pins' must be an array.")
            if not isinstance(groups, list): raise ValueError("'groups' must be an array.")
            if not isinstance(nets,   list): raise ValueError("'nets' must be an array.")
            if not isinstance(colors, list): raise ValueError("'colors' must be an array.")
            # Validate unique pin IDs
...
class Stage3_ColorLabel(BaseStage):
    ...
    def _pick(self, e):
        ...
        iy = max(0, min(S.base.height - 1, int((e.y - oy) * self._scale_y)))
...
class Stage6_Pins(BaseStage):
    ...
    def _delete_selected(self):
        ...
        # Clean up orphan group references
        remaining_ids = {p["id"] for p in S.pins}
        for g in S.groups:
            g["pin_ids"] = [pid for pid in g["pin_ids"] if pid in remaining_ids]
        # Clean up orphan net references
        for n in S.nets:
            n["pin_ids"] = [pid for pid in n["pin_ids"] if pid in remaining_ids]
    ...
    def _do_analysis(self, params: dict):
        try:
            ...
            if k_size % 2 == 0:
                k_size += 1
            k_size = max(1, k_size)
            if not (k_size >= 1 and k_size % 2 == 1):
                k_size = 3
            if mask.any():
...
class Stage9_Export(BaseStage):
    ...
    def _export_zip(self):
        ...
        if not path:
            return
        name = self._get_name()
        # Sanitize name to prevent path traversal or invalid filename characters
        import re as _re
        name = _re.sub(r'[^\w\-]', '_', name) or "pcb_project"
        files_added = []
        try:
