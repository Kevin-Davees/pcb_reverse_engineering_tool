...
======================================================
V5 performance & correctness fixes (vs V4):
────────────────────────────────────────────────────────
PERFORMANCE
...
class AppState:
    ...
    def __init__(self):
        ...
        # Incremented whenever seg_arr is modified so composite caches can
        # detect staleness cheaply without comparing full arrays.
        self.seg_version: int = 0
    ...
    # ── Seg-array delta undo (Stage 4 paint) ──────────────────────────────
    # Instead of snapshotting the entire RGBA array (≈48 MB per step for a
    # 4000×3000 image) we store only the bounding-box region that actually
    # changed.  Memory drops by 95–99% for typical brush strokes.

    def push_seg_undo(self, y1: int, x1: int, patch: "np.ndarray"):
        """Store a minimal region patch for undo.  patch is a *copy* of the
        affected sub-array BEFORE the stroke was applied."""
...
class ZoomMixin:
    ...
    _MIPMAP_LEVELS = 4      # number of pre-scaled mipmap levels to keep
    ...
    def _zoom_init(self, canvas: tk.Canvas):
        ...
        self._zm_pan_x   = None
        self._zm_pan_y   = None
        self._zm_interacting = False    # True while user is dragging/scrolling
        self._zm_idle_id     = None     # after() job id for quality upgrade
        # Mipmap: maps (source_id, level) → PIL image.  Cleared on reset.
        self._zm_mipmap: dict = {}
        self._zm_src_id: int  = 0       # bumped whenever the source image changes
    ...
    def _zm_invalidate_cache(self):
        """Call whenever the source image content changes (e.g. after painting)."""
        self._zm_mipmap.clear()
        self._zm_src_id += 1
    ...
    def _zm_render_image(self, pil_img: "Image.Image") -> "ImageTk.PhotoImage":
        """
        Scale pil_img by current zoom factor, blit to canvas, and return the
        PhotoImage (caller must hold a reference to prevent GC).

        During active interaction (scrolling / panning / painting) uses
        Image.NEAREST for speed.  After IDLE_MS of quiet uses Image.LANCZOS.

        Mipmap cache: for the NEAREST path, caches scaled images keyed by
        (source_id, target_w, target_h) so repeated renders at the same zoom
        level skip the resize entirely.
        """
        iw, ih = pil_img.size
        cw = max(1, self._zm_canvas.winfo_width())
        ch = max(1, self._zm_canvas.winfo_height())
        ...
        if self._zm_interacting:
            # Fast path: check mipmap cache first
            cache_key = (self._zm_src_id, new_w, new_h)
            resized = self._zm_mipmap.get(cache_key)
            if resized is None:
                resized = pil_img.resize((new_w, new_h), Image.NEAREST)
                # Limit cache size to avoid unbounded memory growth
                if len(self._zm_mipmap) > 16:
                    self._zm_mipmap.pop(next(iter(self._zm_mipmap)))
                self._zm_mipmap[cache_key] = resized
        else:
            # Quality path: LANCZOS (only called after idle timeout)
...
class PCBApp(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("PCB Reverse Engineering Tool  v5")
...
class Stage1_Load(ZoomMixin, BaseStage):
    ...
    def _open_image(self):
        ...
        if not path:
            return
        try:
            # FIX: save and restore MAX_IMAGE_PIXELS instead of leaving it permanently raised
...
class Stage2_Perspective(ZoomMixin, BaseStage):
    ...
    def _apply(self):
        ...
        try:
            ...
            warped = cv2.warpPerspective(raw_np, M, (ow, oh), flags=cv2.INTER_LANCZOS4)
...
class Stage3_ColorLabel(ZoomMixin, BaseStage):
    ...
    def _do_segment(self, tol: int, blur: int, blur_type: str):
        try:
            src = S.base.convert("RGB")
            if blur > 0:
                if blur_type == "bilateral":
                    # Bilateral filter preserves sharp trace edges much better
                    # than Gaussian, which can smear thin copper lines.
                    src_np  = np.array(src, dtype=np.uint8)
                    d       = max(3, blur * 2 + 1)   # filter diameter
                    src_np  = cv2.bilateralFilter(src_np, d, sigmaColor=40, sigmaSpace=blur)
                    src     = Image.fromarray(src_np)
                else:
            ...
            # Use squared distance — avoids expensive sqrt, same ordering.
            tol2 = float(tol * tol)
            for c in S.colors:
                r, g, b = c["rgb"]
                ref   = np.array([r, g, b], dtype=np.float32)
                diff  = img_np - ref
                dist2 = np.einsum("...i,...i->...", diff, diff)   # vectorised dot-product
                mask  = dist2 < tol2
            ...
            self.app.after(0, lambda: (
                self._busy.hide(),
                self.status(f"Segmentation complete — {n} colour class(es) mapped.", "success")))
        except Exception as ex:
...
class Stage4_Paint(ZoomMixin, BaseStage):
    ...
    def _composite(self) -> "Image.Image | None":
        """Rebuild composite only when seg_arr or opacity actually changed."""
        if S.base is None:
            return None
    ...
    def _start_stroke(self, e):
        ...
        # Delta undo: record bounding box of stroke before first draw_at.
        # We expand the patch lazily in _draw_at via _stroke_bbox.
        self._stroke_bbox = None   # (y1, x1, y2, x2) accumulated over stroke
    ...
    def _end_stroke(self, _e):
        ...
        if self._stroke_started and S.seg_arr is not None:
            ...
            # Commit the delta-undo entry now that stroke is complete.
            # _stroke_patch was captured before first draw_at in _draw_at.
        ...
        # Trigger high-quality render now that stroke is done
        self._zm_interacting = False
        S.seg_version += 1
        self._zoom_render()
    ...
    def _draw_at(self, cx: float, cy: float):
        ...
        # Delta undo: on first touch of this stroke, save the patch
        # covering the full *final* stroke bounding box by expanding it.
        # We accumulate the bbox and save one copy before the first modification.
        bb = getattr(self, "_stroke_bbox", None)
        new_bb = (y1, x1, y2, x2)
        if bb is None:
            # First touch — save a patch of this cell
            self._stroke_patch_entry = (y1, x1, S.seg_arr[y1:y2, x1:x2].copy())
            self._stroke_bbox = new_bb
        else:
            # Expand bounding box and if it grew, take a fresh pre-draw snapshot
            ny1 = min(bb[0], y1); nx1 = min(bb[1], x1)
            ny2 = max(bb[2], y2); nx2 = max(bb[3], x2)
            if (ny1, nx1, ny2, nx2) != bb:
                # Restore current patch then retake from expanded region
                # (the "before" region must encompass the full stroke)
                if self._stroke_patch_entry is not None:
                    ...
                    # Merge: get full pre-stroke state for the expanded bbox
                    full_patch = S.seg_arr[ny1:ny2, nx1:nx2].copy()
                    # Restore old region to its pre-stroke values
    ...
    def undo(self):
        ...
        if entry is not None:
            ...
            # Save redo delta (current state before restore)
...
class Stage6_Pins(ZoomMixin, BaseStage):
    ...
    def _zoom_render(self, _e=None):
        ...
        self._tk_img = self._zm_render_image(comp)
        # FIX: _zm_render_image calls canvas.delete("all") — any lasso rect is
        # gone after this call. _on_drag will redraw on the next motion event.
    ...
    def _do_analysis(self, params: dict):
        try:
            ...
            for c in S.colors:
                ...
                r, g, b = c["rgb"]
                ref   = np.array([r, g, b], dtype=np.float32)
                diff  = seg[:, :, :3].astype(np.float32) - ref
            ...
            if mask.any():
                ...
                # FIX: dilate first to bridge hairline gaps between traces and pads
            ...
            for p in S.pins:
                ...
                if lbl_id == 0 and snap > 0:
                    ...
                    if nonzero.size > 0:
                        ...
                        # FIX: argmax on counts[1:] to skip background bin 0
