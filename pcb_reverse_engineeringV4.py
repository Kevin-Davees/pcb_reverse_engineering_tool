...
======================================================
V4 changes vs V3 (reliability & output-quality focus):
────────────────────────────────────────────────────────
• Fixed S.__init__() reset — was calling S.__init__(S) (wrong arg)
• Fixed Image.MAX_IMAGE_PIXELS — restored after load, not set globally
• Segmentation: vectorised float32 distance (2× faster, less RAM)
• Segmentation: optional Gaussian blur pre-pass for JPEG/noisy images
• Net analysis: MORPH_DILATE before OPEN/CLOSE bridges hairline gaps
• Net analysis: bincount argmax skips label-0 (background) correctly
• Stage4 undo: push_undo once per stroke, not on each empty-canvas click
• Stage6 lasso: redrawn AFTER _zm_render_image (which clears canvas)
• Stage6 list_select: removed broken tk.call modifier hack
• Stage6 group dialog: warns if pins already grouped
• Stage8 schematic: scrollregion set; large boards scroll instead of overflow
• KiCad PCB export: fixed malformed s-expression; pads now include net label
• ZIP export: kicad_pcb now included in bundle
• BusyBar progress indicator for seg and net analysis
• Guard clauses in all on_enter() — safe to visit stages in any order
• rand_color: golden-ratio HSV stepping for distinct colours
• MAX_UNDO increased 10 → 20; _err() helper logs full traceback
• Tolerance/brush/snap sliders show live numeric value
• Warp uses INTER_LANCZOS4; auto-advances to Stage 3
V3 original features preserved.
─────────────────
...
class PCBApp(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("PCB Reverse Engineering Tool  v4")
    ...
    def _build_ui(self):
        ...
        header.pack(fill="x")
        tk.Label(
            header, text="🔬  PCB Reverse Engineering Tool  v4",
            bg="#161b22", fg="#58a6ff",
...
class Stage3_ColorLabel(ZoomMixin, BaseStage):
    ...
    def _segment(self):
        ...
        self._busy.show()
        threading.Thread(target=self._do_segment, args=(tol, blur), daemon=True).start()

    def _do_segment(self, tol: int, blur: int):
        try:
            src = S.base.convert("RGB")
            if blur > 0:
                src = src.filter(ImageFilter.GaussianBlur(radius=blur))
            # float32: half the memory of int32/float64, vectorised per-colour
...
class Stage4_Paint(ZoomMixin, BaseStage):
    def _build(self):
        ...
        self._stroke_started: bool    = False  # guards undo push
    ...
    def _start_stroke(self, e):
        ...
        # FIX: push undo exactly once per stroke, not on every click
